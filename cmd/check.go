...
func printCheckResultTable(result *CheckResult) {
	...
	if len(result.Workflows.Modified) > 0 {
		fmt.Printf("%sWorkflows Modified in n8n:%s\n", ascii.Bold, ascii.Reset)
	...
	if !alertOnly {
		...
		fmt.Printf("   %s./n8n-ops sync --env %s%s\n", ascii.Dim, environment, ascii.Reset)
		fmt.Printf("   %sgit add workflows/%s/ && git commit -m \"sync: update workflows\"%s\n", 
			ascii.Dim, environment, ascii.Reset)
	}
}
